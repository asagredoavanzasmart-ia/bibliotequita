...
export function ReaderView({ bookId, onClose }: ReaderViewProps) {
  ...
  const speakWithBrowserVoice = useCallback((index: number, phraseList: string[]): boolean => {
    ...
    // 1) La voz elegida por el usuario en "Voz offline" (si sigue disponible
    // en este navegador/dispositivo); 2) si no eligió ninguna, autodetecta
    // una voz local en español; 3) si tampoco hay, cualquier voz en español.
    const chosenVoice =
      (preferredOfflineVoiceURI && voices.find(v => v.voiceURI === preferredOfflineVoiceURI)) ||
    ...
    return true;
  }, [ttsTextSource, handleTtsStop, preferredOfflineVoiceURI]);
